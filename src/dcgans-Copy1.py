...
class Generator(nn.Module):
    def __init__(self,nz=100, ngf=32, nc=1):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(ngf*4, nc, 4, 2, 1, bias=False), #stride=2, padding=1
...
input_tensor = torch.randn(10, 100, 4, 4) 
generator = Generator()
disc  = Discriminator()
output = generator(input_tensor)
print(output.shape)
print(disc(output).shape)  # Should print torch.Size([8, 1, 28, 28])
...
def train(generator, discriminator, latent_dim, train_loader, optimizer, criterion, epochs, device='cpu'):
    ...
    FIXED_NOISE = torch.randn((8, latent_dim,4,4)).to(device)
    ...
    for epoch in range(epochs):
        ...
        for real_imgs, _ in pbar:
            ...
            # generate Noise
            z = torch.randn(real_imgs.shape[0], latent_dim, 4, 4).type_as(real_imgs)
...
train(generator, discriminator, Z_DIM, dataloader, optimizer, GANLoss(), 1000, device)
